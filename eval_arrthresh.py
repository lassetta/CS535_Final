...
fine = None
coarse = None
alp = [.8,.8,.8,.8,.8,.8,.8,.8,.8,.8,.8,.8,.8,.7,.75,.8,.85,.9,.95,1]
...
def eval_net2(dataloader,plotter,alpha):
    ...
    for data in dataloader:
        ...
        alpha = torch.ones(num_iter[0], dtype = torch.float)
        #alpha = alpha.cpu().numpy()
        for i in range(0,num_iter[0]):
            a = top_label_c1[i].cpu().numpy()
            alpha[i] = float(alp[int(a)])
        alpha = alpha.cuda()
...
if __name__ == "__main__":
    alphas = np.loadtxt("alpha1.csv", delimiter ='\n')
    print(alphas)
    alp = alphas
    torch.manual_seed(0)
    np.random.seed(0)
    ...
    for name in fine:
        ...
        for sublist in classes_c2f_map:
            ...
            i = i + 1
    mmap = np.array(a)
    train_l, test_l = split_train(fine,coarse,5)
    ...
    testset = CIFAR100(root='./data', idcs_in = train_l, train=False,
            download=True, transform=transform)

    testloader1 = torch.utils.data.DataLoader(testset, batch_size=BATCH_SIZE,
                                              shuffle=True, num_workers=2)
    ...
    print(unk)
    '''
    train_acc1_c, train_acc1_f, unk, train_acc2_c, train_acc2_f, train_acc3_c, train_acc3_f = eval_net2(trainloader,0,alpha)
    print('Test on same training labels\nCOARSE             FINE\ntest_acc1c: %.5f test_acc1f: %.5f\ntest_acc2c: %.5f  test_acc2f: %.5f\ntest_acc3c: %.5f  test_acc3f: %.5f\n'%
          (train_acc1_c, train_acc1_f, train_acc2_c, train_acc2_f, train_acc3_c, train_acc3_f))
    print(unk)
    '''
    train_acc1_c, train_acc1_f, unk, train_acc2_c, train_acc2_f, train_acc3_c, train_acc3_f = eval_net2(testloader1,0,alpha)
    print('Test on same testing labels\nCOARSE             FINE\ntest_acc1c: %.5f test_acc1f: %.5f\ntest_acc2c: %.5f  test_acc2f: %.5f\ntest_acc3c: %.5f  test_acc3f: %.5f\n'%
          (train_acc1_c, train_acc1_f, train_acc2_c, train_acc2_f, train_acc3_c, train_acc3_f))
    print(unk)
    print('________________________________________________________________________________')
